...
> Note: `truewatch_synthetics_test` is not registered by the provider in the current release branch. This example is kept as implementation reference only and cannot be applied until `truewatch_synthetics_test` is enabled in `provider/resources.go`.
